...
RUN \
  ...
  # Mount optional GitHub token for Git-based gem sources
  ...
  # Authenticate GitHub access for private Git-based gem sources when token is available
  if [ -f /run/secrets/github_token ]; then \
    export BUNDLE_GITHUB__COM="x-access-token:$(cat /run/secrets/github_token)"; \
  fi; \
